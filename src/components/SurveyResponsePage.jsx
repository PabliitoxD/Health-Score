  ...
  return (
    <>
      ...
      <p className="text-slate-400 text-xs mb-5">
        ...
        {rated < ONBOARDING_CATEGORIES.length && (
          <span className="ml-2 text-amber-500 font-medium">
            · {ONBOARDING_CATEGORIES.length - rated} pendente{ONBOARDING_CATEGORIES.length - rated !== 1 ? 's' : ''}
          </span>
        )}
      </p>
      <div className="mb-6 bg-slate-50 rounded-xl px-4 py-1">
  ...
  return (
    <>
      ...
      <p className="text-slate-400 text-xs mb-5">
        ...
        {rated < FOLLOW_CATEGORIES.length && (
          <span className="ml-2 text-amber-500 font-medium">
            · {FOLLOW_CATEGORIES.length - rated} pendente{FOLLOW_CATEGORIES.length - rated !== 1 ? 's' : ''}
          </span>
        )}
      </p>
      <div className="mb-5 bg-slate-50 rounded-xl px-4 py-1">
...
const ThankYou = ({ type, score }) => {
  const isMulti = type === 'csat_onboarding' || type === 'csat_nps_follow';
  return (
    <div className="text-center py-8">
      ...
      <p className="text-slate-500 text-sm leading-relaxed max-w-xs mx-auto">
        Sua opinião é muito importante para nós e nos ajuda a melhorar continuamente sua experiência.
      </p>
      {score != null && (
...
const TYPE_LABEL = {
  ...
  csat_onboarding: 'Pesquisa CSAT — Onboarding',
  csat_nps_follow: 'Pesquisa CSAT — Follow-up',
};

const TYPE_BADGE = {
  nps: 'bg-blue-500/20 text-blue-300',
  csat_health: 'bg-violet-500/20 text-violet-300',
  csat_onboarding: 'bg-violet-500/20 text-violet-300',
  csat_nps_follow: 'bg-rose-500/20 text-rose-300',
};
...
const SurveyResponsePage = () => {
  ...
  // Simple score (nps / csat_health)
  const [score, setScore] = useState(null);
  // Multi-category scores (csat_onboarding / csat_nps_follow)
  const [categoryScores, setCategoryScores] = useState({});
  // NPS Follow open text
  ...
  const handleSubmit = () => {
    ...
    // Sync status back to survey record
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 w-full max-w-lg overflow-hidden">
        ...
        <div className="bg-slate-900 px-8 py-5 flex items-center gap-3">
          ...
          <span className={`ml-auto px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${TYPE_BADGE[type] || 'bg-slate-500/20 text-slate-300'}`}>
